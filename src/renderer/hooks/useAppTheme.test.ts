...
describe('getThemeDesign', () => {
  it('uses the interface brand color for MUI primary controls', () => {
    expect(getThemeDesign('light').palette?.primary).toEqual({ main: '#c026d3', contrastText: '#000000' })
    expect(getThemeDesign('dark').palette?.primary).toEqual({ main: '#f472d0', contrastText: '#000000' })
